<div class="space-y-6">
    ...
    <div class="va-card rounded-2xl overflow-hidden shadow-2xl mt-6 border" style="background-color: var(--tenant-card-bg, #181D29); border-color: var(--tenant-input-border, rgba(255,255,255,0.12)); color: var(--tenant-card-text, #ffffff);">
        <div class="px-6 py-5 border-b flex justify-between items-center flex-wrap gap-4" style="background-color: var(--tenant-panel-bg, #141923); border-color: var(--tenant-input-border, rgba(255,255,255,0.1));">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <div class="relative flex items-center">
                    ...
                    @if($csvFile)
                        <button wire:click="importCsv" class="btn-primary px-4 py-2 rounded-xl text-sm font-semibold shadow-sm transition">
                            Import
                        </button>
                    @endif
                </div>
                <a href="{{ route('global-network') }}" class="btn-secondary px-4 py-2 rounded-xl text-sm font-semibold shadow-sm transition flex items-center gap-2 border border-tenant-accent/40 text-tenant-accent">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    Global Network Import
                </a>
                <button wire:click="openAddModal" class="btn-primary px-4 py-2 rounded-xl text-sm font-semibold shadow-sm transition flex items-center gap-2">
        ...
        @if(count($selectedRoutes) > 0)
            <div class="px-6 py-3.5 bg-tenant-accent/20 border-b border-tenant-accent/40 flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    <button wire:click="openMassUpdateModal" class="btn-primary px-4 py-2 rounded-xl text-xs font-bold shadow-md transition flex items-center gap-1.5">
            ...
            <table class="min-w-full divide-y" style="border-color: var(--tenant-input-border, rgba(255,255,255,0.08));">
                <thead style="background-color: rgba(0,0,0,0.12); border-bottom: 1px solid var(--tenant-input-border, rgba(255,255,255,0.08));">
                    <tr>
                        ...
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style="color: var(--tenant-card-muted, #94a3b8);">Flight Number</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style="color: var(--tenant-card-muted, #94a3b8);">ATC Callsign</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style="color: var(--tenant-card-muted, #94a3b8);">Departure</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style="color: var(--tenant-card-muted, #94a3b8);">Arrival</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style="color: var(--tenant-card-muted, #94a3b8);">Block Time</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style="color: var(--tenant-card-muted, #94a3b8);">Route Type</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style="color: var(--tenant-card-muted, #94a3b8);">Aircraft Types</th>
                ...
                <tbody class="divide-y" style="border-color: var(--tenant-input-border, rgba(255,255,255,0.05));">
                    ...
                    <tr class="hover:bg-white/5 transition-colors">
                        ...
                        <td class="p-4 text-center">
                            <input type="checkbox" wire:model.live="selectedRoutes" value="{{ $route->id }}" class="rounded bg-black/50 border-gray-600 text-tenant-accent focus:ring-tenant-accent cursor-pointer">
                        </td>

                        <!-- Flight Number (Commercial / IATA) -->
                        <td class="px-5 py-4 whitespace-nowrap text-sm font-bold font-mono" style="color: var(--tenant-card-text, #ffffff);">
                            {{ $route->flight_number }}
                        </td>
                        ...
                        <!-- Block Time -->
                        <td class="px-5 py-4 whitespace-nowrap text-sm font-mono font-medium" style="color: var(--tenant-card-text, #cbd5e1);">
                            {{ $route->block_time }}
                        </td>
    ...
    <x-dialog-modal wire:model.live="showAddModal">
        ...
        <x-slot name="content">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="col-span-1">
                    <x-label for="flight_number" value="{{ __('Flight Number (e.g. U28161 / FR2605 / 1181)') }}" class="font-semibold text-xs uppercase tracking-wider mb-1.5" style="color: var(--tenant-card-text, #ffffff);" />
                    <x-input id="flight_number" type="text" class="mt-1 block w-full uppercase font-mono font-bold" wire:model="flight_number" placeholder="e.g. U28161 or 1181" />
                    <p class="text-[11px] mt-1" style="color: var(--tenant-card-muted, #94a3b8);">Commercial flight identifier (IATA/number).</p>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="col-span-1">
                    <x-label for="departure_icao" value="{{ __('Departure ICAO') }}" class="font-semibold text-xs uppercase tracking-wider mb-1.5" style="color: var(--tenant-card-text, #ffffff);" />
                    <x-input id="departure_icao" type="text" class="mt-1 block w-full uppercase font-mono font-bold" wire:model="departure_icao" placeholder="e.g. EGLL" maxlength="4" />
                    <x-input-error for="departure_icao" class="mt-2 text-red-400 text-xs" />
                </div>
                <div class="col-span-1">
                    <x-label for="arrival_icao" value="{{ __('Arrival ICAO') }}" class="font-semibold text-xs uppercase tracking-wider mb-1.5" style="color: var(--tenant-card-text, #ffffff);" />
                    <x-input id="arrival_icao" type="text" class="mt-1 block w-full uppercase font-mono font-bold" wire:model="arrival_icao" placeholder="e.g. LFPG" maxlength="4" />
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="col-span-1">
                    <x-label for="block_time" value="{{ __('Block Time (HH:MM)') }}" class="font-semibold text-xs uppercase tracking-wider mb-1.5" style="color: var(--tenant-card-text, #ffffff);" />
                    <x-input id="block_time" type="text" class="mt-1 block w-full font-mono" wire:model="block_time" placeholder="e.g. 02:30 (Auto if blank)" />
                    <x-input-error for="block_time" class="mt-2 text-red-400 text-xs" />
                </div>
                <div class="col-span-1">
                    <x-label for="distance" value="{{ __('Distance (NM)') }}" class="font-semibold text-xs uppercase tracking-wider mb-1.5" style="color: var(--tenant-card-text, #ffffff);" />
                    <x-input id="distance" type="number" class="mt-1 block w-full font-mono" wire:model="distance" placeholder="Auto if blank" />
            ...
            <div class="col-span-6 sm:col-span-4 mb-4">
                <x-label for="route_string" value="{{ __('Route String (Optional)') }}" class="font-semibold text-xs uppercase tracking-wider mb-1.5" style="color: var(--tenant-card-text, #ffffff);" />
                <x-input id="route_string" type="text" class="mt-1 block w-full font-mono uppercase text-sm" wire:model="route_string" placeholder="e.g. DCT BOVIS L608..." />
                <p class="text-xs mt-1" style="color: var(--tenant-card-muted, #94a3b8);">Leave empty to have SimBrief calculate the route automatically.</p>
                ...
                <div class="mt-2 grid grid-cols-2 gap-2 max-h-36 overflow-y-auto rounded-xl p-3 border" style="background-color: var(--tenant-input-bg, #0a0d14); border-color: var(--tenant-input-border, rgba(255,255,255,0.15));">
                    @foreach($aircraftTypes as $type)
                        <label class="inline-flex items-center cursor-pointer">
                            ...
                            <span class="ml-2 text-sm font-mono font-medium" style="color: var(--tenant-card-text, #ffffff);">{{ $type->code }} - {{ $type->name }}</span>
                        </label>
                    @endforeach
                </div>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showAddModal', false)" wire:loading.attr="disabled" class="btn-secondary px-5 py-2.5 rounded-xl font-bold text-sm">
                {{ __('Cancel') }}
            </x-secondary-button>

            <button wire:click="saveRoute" wire:loading.attr="disabled" class="ml-3 btn-primary px-6 py-2.5 rounded-xl text-sm font-bold shadow-md transition">
    ...
    <x-dialog-modal wire:model.live="showMassUpdateModal">
        ...
        <x-slot name="content">
            <p class="text-xs mb-4" style="color: var(--tenant-card-muted, #94a3b8);">
                Update the ATC Airline ICAO Prefix and re-generate full callsigns for all <strong>{{ count($selectedRoutes) }}</strong> selected routes.
            </p>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div class="col-span-1">
                    <x-label for="massStripPrefix" value="{{ __('Flight # Prefix to Cut Off (Optional)') }}" class="font-semibold text-xs uppercase tracking-wider mb-1.5" style="color: var(--tenant-card-text, #ffffff);" />
                    <x-input id="massStripPrefix" type="text" wire:model="massStripPrefix" placeholder="e.g. U2 or FR (auto if blank)" class="block w-full uppercase font-mono" />
                </div>
            </div>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showMassUpdateModal', false)" wire:loading.attr="disabled" class="btn-secondary px-5 py-2.5 rounded-xl font-bold text-sm">
                {{ __('Cancel') }}
            </x-secondary-button>

            <button wire:click="applyMassUpdate" wire:loading.attr="disabled" class="ml-3 btn-primary px-6 py-2.5 rounded-xl text-sm font-bold shadow-md transition">
