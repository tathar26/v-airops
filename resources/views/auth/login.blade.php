...
<head>
    ...
    <title>Pilot Sign In | V-Air Ops Platform</title>
...
<body class="h-full flex items-center justify-center p-4 bg-[#0A1835] overflow-x-hidden">
    ...
    <div class="w-full max-w-md space-y-6 my-auto">
        ...
        <div class="text-center space-y-2">
            ...
            <h1 class="text-2xl font-extrabold tracking-tight text-white">Pilot Sign In</h1>
            <p class="text-slate-400 text-sm">Access your virtual airline cockpit and pilot console</p>
        </div>
        ...
        <div class="bg-[#0F224A] border border-[#142954] rounded-2xl p-8 shadow-2xl space-y-6">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-xs text-[#21A19D] hover:text-teal-300 transition-colors">
                                Forgot password?
                            </a>
                        @endif
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <label for="remember_me" class="flex items-center gap-2.5 text-xs text-slate-400 cursor-pointer select-none">
                        <input id="remember_me" type="checkbox" name="remember" class="w-4 h-4 rounded bg-[#060E22] border-[#142954] text-[#21A19D] focus:ring-[#21A19D] focus:ring-offset-[#0F224A]">
            ...
            <div class="pt-4 border-t border-[#142954] text-center text-xs text-slate-400">
                Don't have an account? <a href="{{ route('register') }}" class="text-[#21A19D] hover:text-teal-300 font-semibold underline underline-offset-4">Create free pilot account</a>
            </div>
        </div>
    </div>
